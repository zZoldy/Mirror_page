...
public class jInternal_tabela extends javax.swing.JInternalFrame {
    ...
    void event_enter() {
        // se ainda está em edição, ignora esse ENTER
        ...
        // ENCERRAMENTO DO JORNAL (aqui você usou last_row, mas talvez seja penúltima)
    ...
    private void tabela_newsKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tabela_newsKeyPressed
                ...
                new SwingWorker<Void, Void>() {
                    ...
                    protected void done() {
                        try {
                            ...
                            Log.registrarErro_noEx("[INSERT] - Linha não inserida");
                        }
                    }
                }.execute();
            }, acao_line);
        ...
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            // 2) NÃO está editando → tenta TRAVAR e entrar em edição
    ...
    void edicao_celula() {
        ...
        // 🔁 Listener global de início/fim de edição (só precisa registrar UMA vez)
            ...
            if (newEditor instanceof TableCellEditor editor) {
                editor.addCellEditorListener(new javax.swing.event.CellEditorListener() {
                    ...
                    public void editingStopped(javax.swing.event.ChangeEvent e) {
                        // edição confirmada (ENTER, clique fora, TAB, etc.)
                        onEditingFinished(true);
                    }

                    @Override
                    public void editingCanceled(javax.swing.event.ChangeEvent e) {
                        // edição cancelada (ESC)
        ...
        // ===== ENTER: iniciar edição OU finalizar edição =====
        ...
        am_enter.put("start-or-commit-edit", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // 1) Se JÁ está editando → ENTER só faz COMMIT
                // (o unlock será disparado pelo CellEditorListener.editingStopped)
                if (tabela_news.isEditing()) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        if (!lockResult.granted) {
                            ...
                            SwingUtilities.invokeLater(() -> {
                                JOptionPane.showMessageDialog(
                                        tabela_news,
                                        "Esta célula está sendo editada por: " + dono,
                                        "Célula bloqueada",
                        ...
                        // LOCK OK → entra em edição na EDT
                        SwingUtilities.invokeLater(() -> {
                            lockedRow = modelRow;
                            lockedCol = sheetCol;
                        ...
                        SwingUtilities.invokeLater(() -> {
                            JOptionPane.showMessageDialog(
                                    tabela_news,
                                    "Erro ao tentar bloquear a célula.\n" + ex.getMessage(),
                                    "Erro de comunicação",
        ...
        // ===== ESC: cancelar edição (voltar valor antigo) =====
        ...
        am_esc.put("cancel-edit", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // ESC cancela a edição atual → valor volta ao que era antes
                // e dispara editingCanceled → onEditingFinished(false) → unlock
                TableCellEditor editor = tabela_news.getCellEditor();
                if (editor != null) {
    ...
    private void onEditingFinished(boolean committed) {
        ...
        // 🔒 Garante que só a PRIMEIRA chamada vai executar unlock
        synchronized (this) {
            if (editingModelRow < 0 || editingModelCol < 0) {
                // Já processamos/unlock está em andamento
                return;
            }

            // Captura os índices atuais
            row = editingModelRow;
            col = editingModelCol;

            // Já limpa o estado AQUI para evitar chamadas duplicadas
            clearEditingState();
        }
        ...
        // Faz o unlock em background com os índices capturados
        new Thread(() -> {
            try {
                ...
                if (msg != null
                    ...
                    // Já estava liberado / lock expirou / outra tentativa duplicada
                    System.out.println("[EDITOR] unlock 403 Not lock owner (já não havia lock). Ignorando.");
                } else {
                    System.err.println("[EDITOR] Erro HTTP ao desbloquear célula:");
                    ex.printStackTrace();
                }
            } catch (Exception ex) {
                System.err.println("[EDITOR] Erro inesperado ao desbloquear célula:");
                ex.printStackTrace();
            }
        }).start();
    }
    ...
    private void conectarWebSocket() {
        try {
            ...
            String topicId = csv_server
                    .replace("\\", "/")
                    .replace("/", "_");   // /BDBR/Final.csv → _BDBR_Final.csv
    ...
    private void onCellChangeEvent(CellChangeEvent evt) {
        ...
        // 1) Garante que é desta planilha
        ...
        javax.swing.SwingUtilities.invokeLater(() -> {
            ...
            if (viewRow < 0 || viewCol < 0) {
                System.out.printf("[WS] viewRow/viewCol inválidos: %d/%d%n", viewRow, viewCol);
                return;
            }

            System.out.printf(
                    "[WS] Aplicando mudança externa: path=%s row=%d col=%d value='%s' user=%s%n",
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                // (Opcional) aqui dá pra fazer highlight na célula, etc.
            });
    ...
    private void onRowInsertedEvent(RowInsertedEvent ev) {
        ...
        System.out.println("[WS] Linha inserida após: " + ev.afterRow());
    ...
    void add_line(int viewRow, int viewCol) {
        ...
        // última linha de dados (penúltima da tabela: antes da TOTAL)
        ...
        // posição no model onde a nova linha será inserida
        ...
        if (prevRow >= 0 && prevRow < modelo.getRowCount()) {
            ...
            if (val != null) {
                try {
                    ...
                    // se não for número, mantém 1 (ou outro padrão se quiser)
                }
            }
        }

        // monta a linha vazia com seus padrões
        Object[] novaLinha = new Object[]{
            numPag, // primeira coluna = sequência
            "", "", "", "", "", "", "",
            "00:00", "00:00", "00:00",
            "", "", "00:00:00", ""
        };
